  ...
  return (
    <div className="min-h-screen bg-[#2d2d2d] text-white font-sans relative">
      ...
      <header className="p-6 flex justify-between items-center bg-[#2d2d2d] sticky top-0 z-50 h-20">
        {!isSearchOpen ? (
          <>
            <button onClick={() => setIsMenuOpen(true)} className="text-3xl hover:text-[#48daff] transition-colors">☰</button>
            <span className="font-bold text-2xl tracking-wide">GAMEVERSE</span>
            <button onClick={() => setIsSearchOpen(true)} className="text-3xl hover:text-[#48daff] transition-colors">🔍</button>
          </>
        ) : (
      ...
      {/* CARRUSEL FUNCIONAL CON FLECHAS RESTAURADAS */}
      {!searchQuery && (
      ...
      <main className="px-6 pb-20">
        {filteredGames.length > 0 ? (
          <>
            <GameRow title={searchQuery ? "RESULTADOS DE BÚSQUEDA" : "POPULARES"} games={filteredGames.slice(0, 4)} />
          ...
          <div className="text-center py-20 animate-fade-in">
            <h2 className="text-2xl text-gray-400">No hay coincidencias para "{searchQuery}"</h2>
          </div>
        )}
      </main>
    </div>
...
const GameRow = ({ title, games }: { title: string, games: any[] }) => (
  <section className="mb-12">
    <h3 className="text-2xl font-bold mb-6 tracking-wide uppercase border-l-4 border-[#48daff] pl-4">{title}</h3>
